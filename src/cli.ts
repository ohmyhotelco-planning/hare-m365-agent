...
- loggedIn이 false이면 사용자에게 어떤 명령을 실행할지 묻지 말고 auth login을 먼저 실행합니다.
- auth login 중 터미널에 표시되는 Microsoft device code는 사용자가 직접 브라우저에 입력합니다. LLM은 코드를 채팅으로 복사하거나 반복하지 않습니다.
- 현재 환경에서 사용자가 로그인 화면과 코드를 볼 수 없으면 우회하지 말고 정확한 로컬 로그인 명령을 안내하고 멈춥니다.
...
const llmPrompt = `Hare M365 Agent를 사용해 내 Microsoft 365 요청을 처리해. Claude/Cowork처럼 도메인 허용 목록이 있는 환경이면 먼저 github.com, release-assets.githubusercontent.com, registry.npmjs.org, graph.microsoft.com, login.microsoftonline.com 허용 여부를 사용자에게 확인해. 도메인 허용 완료 전에는 npm exec나 조회를 시작하지 마. 이후 llm-guide를 읽고 doctor/auth status로 설정과 로그인을 확인해. loggedIn이 false이면 사용자에게 선택지를 묻지 말고 auth login을 먼저 실행해. Microsoft device code는 사용자가 화면에서 직접 보고 브라우저에 입력하게 하고, 코드를 채팅에 복사하거나 반복하지 마. .env, .cache, token, device code는 읽거나 출력하지 마.`;
      ...
      JSON.stringify(
        {
          ...
          llmInstruction:
            "Read llm-guide, confirm configured/loggedIn status, and use the smallest safe read command needed for the user's request. Never read or print .env, .cache, tokens, or device codes."
        },
        null,
        2
      )
