...
- Persistent store: the connected HareM365Agent project folder
...
Start every Cowork task with the existing HareM365Agent project folder selected. The connected folder is the persistent store; the data directory above is where Hare actually runs.
Cowork Linux defaults such as /root/.local/share are container-local and are never a persistent Hare data directory.
Pick the run location by inspection, not by assumption: if the connected-folder mount (a path shaped like /sessions/<session>/mnt/HareM365Agent) is directly visible and writable in the network-enabled shell, use that mount root as the data directory. If it is not visible there (standard Cowork), use the hybrid model: run Hare in a fixed sandbox path (for example /home/claude/hare) as the data directory, and sync persistent files with the connected folder using the file staging/commit tools.

## Hybrid session start and sync

1. Stage from the connected folder into the data directory (same relative paths): .hare-app-snapshot.tar.gz, .hare-app-build-head, .cache/msal-cache.json, and the claude/ documents.
...
3. After any rebuild, commit the refreshed .hare-app-snapshot.tar.gz and .hare-app-build-head back to the connected folder. After any login or token-cache change, commit .cache/msal-cache.json back as well. Results the user wants to keep go back under results/.
...
6. Do not interpret a session-local /sessions or /tmp data directory as the user's persistent login state. Accept /sessions/<session>/mnt/HareM365Agent as the connected host folder when it is directly visible in the running shell.
