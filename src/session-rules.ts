...
Do not create or select a different Hare data folder. In a new Cowork chat, invoke the folder-access tool for the existing host HareM365Agent folder so the user only approves access. Do not ask the user to browse for it. Use its mounted root as the data directory above.
...
5. FOLDER_REQUIRED means the persistent host folder is not mounted. Invoke the folder-access request for the exact HareM365Agent host path first. Only if the path does not exist, use computer-use to create it and retry the same request once. Use manual instructions only when the required tools are unavailable.
