...
- Without --mailbox, Outlook commands cover only the signed-in user's mailbox. When the user explicitly names a shared mailbox, pass that name or exact address through --mailbox on outlook recent, flagged, search, or count. If name resolution is ambiguous, ask for the exact address. Never fall back to the signed-in user's mailbox after a shared-mailbox error.
- When the user asks for an Outlook attachment, use outlook attachments list with the message ID, then outlook attachments download with the selected attachment ID. For a shared-mailbox message, pass the same --mailbox value to both attachment commands. Downloaded files are saved under Hare downloadsDir and remain subject to the download policy and size limit.
...
- For a full SharePoint site file export, run sharepoint export-files without an approval token first. Show the complete batch preview including site, destination, counts, total bytes, existing files, oversized exclusions, disk space, and limits, then stop.
- If SharePoint export returns ENUMERATION_IN_PROGRESS, rerun the exact command without an approval token so the saved Graph-page checkpoint can continue. Do not ask for approval until the complete batch preview is available.
- After explicit approval, rerun the exact command once with its --approval-token. The unchanged approved manifest can resume for 24 hours without per-file approvals. Never overwrite existing files or bypass the 10GiB per-file export cap.
