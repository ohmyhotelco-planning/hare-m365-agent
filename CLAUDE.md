...
3. HARE_ROOT 판별: 네트워크가 되는 셸에서 연결 폴더 마운트(`/sessions/<session>/mnt/HareM365Agent`)가 직접 보이면 그 마운트를 HARE_ROOT로 사용합니다. 보이지 않으면 하이브리드 모델로, 샌드박스 고정 경로(예: `/home/claude/hare`)를 HARE_ROOT로 쓰고 연결 폴더는 영속 저장소로 동기화합니다.
