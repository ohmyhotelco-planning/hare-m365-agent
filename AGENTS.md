...
영속 저장소는 폴더 이름으로 검색하지 않고 현재 Cowork 작업에 선택된 프로젝트 루트로 확정합니다. 이름이 무엇이든 현재 선택된 프로젝트 하나만 사용하며, 비슷한 이름의 다른 로컬 폴더로 전환하지 않습니다. 네트워크가 되는 실행 셸에서 현재 프로젝트 마운트(`/sessions/<session>/mnt/<selected-project>` 형태)가 직접 보이고 쓰기 가능하면 그 마운트 루트를 HARE_ROOT와 `--data-dir`로 사용합니다(단일 환경). 보이지 않으면 하이브리드 모델을 사용합니다: 샌드박스 고정 경로(예: `/home/claude/hare`)를 HARE_ROOT로 쓰고, 현재 선택된 프로젝트 루트는 영속 저장소로 사용해 파일 스테이징/커밋 도구로 동기화합니다.
...
- 사용자는 Cowork 작업을 만들 때 기존 Hare 프로젝트 또는 폴더를 먼저 선택합니다. 이후에는 그 실제 선택 프로젝트 루트를 영속 저장소로 사용하고 폴더 이름을 추측하지 않습니다. 모든 Hare 명령은 판별된 HARE_ROOT 하나를 동일한 `--data-dir`로 사용합니다.
...
- 일반 `/sessions`, `/tmp` 경로는 로그인에 사용하지 않습니다. `/sessions/<session>/mnt/<selected-project>`는 현재 선택된 로컬 프로젝트의 정상 마운트 경로이므로 실행 셸에서 직접 보이면 그대로 사용합니다.
