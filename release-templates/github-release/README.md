...
이미 도메인 허용과 로그인을 완료했다면 새 채팅용 프롬프트만으로 충분합니다. 인증이 만료되었거나 캐시가 없는 환경이면 LLM이 사용자에게 선택지를 묻지 않고 `auth login`을 먼저 실행합니다. 사용자는 화면의 Microsoft device code를 브라우저에 직접 입력하고, 완료 후 "로그인 완료"라고 알려주면 됩니다.
