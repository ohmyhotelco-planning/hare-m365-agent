...
LLM -> Cowork sandbox shell(도메인 허용 목록 적용) -> 스냅샷 복원 또는 git clone -> HEAD 검증 -> 필요 시 npm ci + npm run build -> Hare CLI -> Microsoft Graph delegated access
                     ↕ 파일 스테이징/커밋 도구
현재 Cowork 작업에 선택된 프로젝트 루트(영속 저장소: 스냅샷, 빌드 마커, 로그인 캐시, 규칙, 결과)
```

Cowork에는 실행 셸이 두 곳 있을 수 있습니다. 도메인 허용 목록은 세션 샌드박스 셸에 적용되고, 선택 프로젝트가 마운트된 디바이스 셸은 설정과 무관하게 모든 외부 도메인이 차단될 수 있습니다. 네트워크가 필요한 명령은 샌드박스 셸에서 실행하고, 현재 선택된 프로젝트 루트는 영속 저장소로 사용합니다. 폴더 이름으로 다른 저장소를 검색하지 않습니다. 프로젝트 마운트가 네트워크가 되는 셸에서 직접 보이는 환경이라면 그 마운트를 실행 위치로 함께 사용해도 됩니다.
...
- 현재 선택된 프로젝트 루트와 HARE_ROOT 간 스냅샷·로그인 캐시·규칙·결과 동기화
