...
도메인 허용 완료 전에는 폴더 연결 요청도 시작하지 마. 도메인 허용 후 먼저 현재 실행환경에서 llm-guide, doctor, auth status를 시도해. 그 과정에서 폴더 접근 권한 또는 지속 인증 캐시 위치가 필요하다고 확인될 때만 Claude/Cowork의 폴더 연결 기능으로 문서/Documents 안의 Hare M365 Agent 전용 폴더 연결/허용을 요청해. 폴더 존재 여부를 미리 검사해서 사용자에게 만들라고 요구하지 마.
...
폴더 연결 요청은 먼저 하지 마. 현재 실행환경에서 llm-guide, doctor, auth status를 먼저 시도하고, 폴더 접근 권한 또는 지속 인증 캐시 위치가 필요하다고 확인될 때만 Claude/Cowork의 폴더 연결 기능으로 문서/Documents 안의 Hare M365 Agent 전용 폴더 연결/허용을 요청해.
